<?php
    ...
    $masuk = mysqli_query($konek, "SELECT * FROM table_siswa WHERE kelas IN('X')");
    
    $tugas = mysqli_query($konek, "SELECT * FROM guru WHERE status = 'enable' AND kelas IN ('X') ");

    $ambil = $_SESSION['nis'] ?? null;
    if (!$ambil) {
    die("NIS tidak ditemukan. Silakan login ulang.");
    }
    ...
    $query = "INSERT INTO tugas_siswa (nis_siswa, nama_siswa, file_tugas, keterangan, jenis, kelas , tanggal) VALUES ('$ambil','$nama', '$namaFileBaru','$keterangan', '$jenis', '$kelas', '$tanggal' )";
    mysqli_query($konek,$query);
    echo "<script>alert('Tugas berhasil diupload');
    window.location.href='klsX.php'
    </script>";
    }
...
<body class="bg bg-secondary">
     <nav class="sidebar navbar bg-dark navbar-dark w-25  d-flex flex-column p-3 h-100">
        ...
        <div id="sidebarMenu" class="collapse d-md-block w-100">
            ...
            <div class="button d-flex flex-column my-4 gap-5">
                <a href="klsX.php" class="btn btn-info btn-lg text-light"><i class="bi bi-house-dash"></i>Home</a>
                <a href="./kelas10/tugasKlsX.php" class="btn btn-outline-info text-light">Cek Tugas Yang diupload</a>
                <a href="./kelas10/tugasGuru.php" class="btn btn-outline-info text-light">Lihat Tugas atau Ujian </a>
                <a href="./kelas10/dokumen.php" class="btn btn-outline-info text-light">Lihat Dokumen</a>
    ...
    <div class="alert d-flex flex-row justify-content-evenly gap-5" role="alert">
      <strong class="fs-1 text-light bg-info rounded-2 w-75 text-center">Tabel Siswa Kelas X</strong>
